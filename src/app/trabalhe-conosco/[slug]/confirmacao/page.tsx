...
export default async function ConfirmacaoEvento({ params }: Props) {
  const { slug } = await params

  const sql = getDb()
  const rows = await sql`SELECT nome, cidade FROM eventos WHERE slug = ${slug}`
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-4">
        <div className="text-center max-w-lg">
          ...
          <h1 className="text-3xl font-bold text-mega-navy mb-2">
            Cadastro Enviado com Sucesso!
          </h1>
          <p className="text-mega-teal font-semibold mb-4">
